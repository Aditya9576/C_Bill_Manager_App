...
Here you can add data , delete data , 1 setp undo on the deleted data , display data, search any particular field and can also print the whole data in a new file.
...
see in the display section "ram's" data has been removed.

D.Now if we want to get back the "ram's" data back ,  go to the undo section to do so.

E: To serach any particular field , just enter the name attribute of that field. 

F: Finally if we want ot get these data in  a separate file ,the simply go to the print section and enter a name for your file and click enter. 
Now see in my case file is get stored inside "/Bill/SavedFiles" folder.

{ In order to run the program , make sure to place the main folder(Bill folder) on desktop. However you can change the location but you must have to copy the location address and paste in the SetPath.txt file
 inside "Bill/bill_exe/SetPath.txt" . }